...
WPITEMPDIR=/tmp/WPITEMP-table-wrapper-csv-impl
...
rm -rf ${WPITEMPDIR}
echo "Removed ${WPITEMPDIR}"
mkdir -p ${WPITEMPDIR}
echo "Created ${WPITEMPDIR}"

echo "Checking for the first time"
echo "------------------------"
${CLEAN_CMD}
${BUILD_CMD} > preCheck.out # Compile the program so that WPIOUTDIR is created.
${CLEAN_CMD}
echo "------------------------"
